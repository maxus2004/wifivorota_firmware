...
void wanClient_restart(){
    ...
    sprintf(apiURI, "wss://vorota.servermaksa.tk/api/?login=%s&version=%s",config_getUserLogin(),config_getVersion());
...
void wanClient_stream_task(void* pvParameters) {
    sprintf(streamURI, "wss://vorota.servermaksa.tk/stream/?login=%s&version=%s",config_getUserLogin(),config_getVersion());
